...
import {
  FiLayout, FiShoppingBag, FiChevronLeft, FiChevronRight, FiLogOut, FiZap
} from 'react-icons/fi';

const sidebarItems = [
  { icon: FiLayout, label: 'Dashboard', href: '/operator' },
  { icon: FiShoppingBag, label: 'Buyurtmalar', href: '/operator/orders' },
  { icon: FiZap, label: 'Real-time', href: '/operator/orders/realtime' },
];
...
export default function OperatorLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!user || !['operator', 'senior_operator', 'admin', 'super_admin'].includes(user.role)) {
    return null;
  }
    ...
    <div className="min-h-screen flex pt-20">
      ...
      >
        <div className="h-full glass-card rounded-none border-l-0 border-t-0 border-b-0 p-4 flex flex-col">
          ...
          {/* Operator badge */}
          <div className="px-3 py-2 mb-4 rounded-xl bg-[#A855F7]/10 border border-[#A855F7]/20">
            <p className="text-xs text-[#A855F7] font-medium">Operator panel</p>
          ...
          <nav className="flex-1 space-y-1">
            {sidebarItems.map((item) => {
              ...
              return (
                ...
                >
                  <item.icon className={`w-5 h-5 flex-shrink-0 ${isActive ? 'text-[#A855F7]' : ''}`} />
